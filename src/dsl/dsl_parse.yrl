...
%%% @doc
%%% This document is just a slightly altered version of erl_parse.erl from
%%% Erlang/OTP distribution. The changes have been highlighted in the source
%%% code.
%%% @end
...
% lewis grozinger
'become' 'receiving' 'spontaneous' 'alarm' 'nout' 'nin' 'send' 'to'
dot.
...
rule -> state '*' event expr_max clause_body : build_rule('$1', '$3', '$4', '$5').
    
state -> atom : '$1'.
event -> 'receiving'   : {event, 'receiving'}.
...
%% lewis grozinger
expr_100 -> 'send' expr_150  'to' expr_150 : build_sendto('$2', '$4').
%%
expr_100 -> expr_150 : '$1'.
...
%% lewis grozinger
expr_max -> 'become' atom : build_become('$2').
expr_max -> 'nin' : {nin}.
expr_max -> 'nout' : {nout}.
%%
expr_max -> if_expr : '$1'.
...
%% lewis grozinger
-type af_state() :: {state, atom()}.

-type af_event() :: {event, 'receiving'}
		  | {event, 'spontaneous'}
		  | {event, 'alarm'}.

-type af_actions() :: {actions, af_cons(abstract_expr())}.

-type af_become() :: {become, af_state()}.

-type af_sendto() :: {sendto, 
		      {msg, abstract_expr()}, 
		      {recipients, af_cons(af_atom())}}.
...
%%--------------------------------------------------------------------
%% @doc
%% Seperates a list of Tokens generated by either erl_scan or dsl_scan 
%% into a list of lists of tokens, with each element in the top level
%% list representing exactly one Erlang form, so that it can be parsed
%% by erl_parse:parse/1
%% @param Tokens a list of tokens generated by erl_scan or dsl_scan.
%% @returns the same list but split at {dot, Any} tokens so that it
%% is a list of lists of form tokens.
%% @end
%%--------------------------------------------------------------------
-spec split(Tokens) -> [FormTokens] when
      Tokens     :: erl_scan:tokens(),
      FormTokens :: [erl_scan:tokens()].
...
form_scan([], FormsList, []) ->
    lists:reverse(FormsList); 

form_scan(FormList, FormsList, []) -> 
    lists:reverse([lists:reverse(FormList)|FormsList]);

form_scan(FormList, FormsList, [H|T]) -> 
    case H of
	{dot, _} -> 
	    NewForm = lists:reverse([H|FormList]),
	    form_scan([], [NewForm|FormsList], T);
	________ ->
	    form_scan([H|FormList], FormsList, T)
    end.
	    
%%--------------------------------------------------------------------
%% @doc
%% Builds a 'send ... to' abstract syntax representation.
%% @param Message the message to be sent.
%% @param Recipient the intended recipient/s of Message.
%% @returns the abstract form representation of send to expression.
%% @end
%%--------------------------------------------------------------------
-spec build_sendto(Message, Recipients) -> ASR when
      Message    :: abstract_expr(),
      Recipients :: af_cons(af_atom()),
      ASR        :: af_sendto().

build_sendto(Message, Recipients) ->
    cons = element(1, Recipients),
    {sendto, {msg, Message}, {recipients, Recipients}}.

%%--------------------------------------------------------------------
%% @doc
%% Builds a 'become' abstract syntax representation.
%% @param NewStatus the atom naming the status to become.
%% @returns the abstract form representation of a become expression.
%% @end
%%--------------------------------------------------------------------
-spec build_become(NewState) -> ASR when
      NewState :: af_atom(),
      ASR :: af_become().

build_become(NewState) ->
    {become, build_state(NewState)}.

%%--------------------------------------------------------------------
%% @doc
%% Builds a state tuple from an atom one.
%% @param Atom the atom tuple containing the name of the required state.
%% @returns a state tuple used as the abstract syntax representation for
%% a state.
%% @end
%%--------------------------------------------------------------------
-spec build_state(Atom) -> ASR when 
      Atom :: {atom, Line, Name},
      Name :: atom(),
      Line :: integer(),
      ASR  :: af_state(). 
...
build_state(Atom) ->
    atom = element(1, Atom),
    {state, element(3, Atom)}. 

%%--------------------------------------------------------------------
%% @doc
%% Builds an event tuple from an atom tuple.
%% @param the ASR for an atom with the name of the category of event.
%% @returns an event tuple as used for the abstract syntax representation for
%% an event.
%% @end
%%--------------------------------------------------------------------
-spec build_event(Atom) -> ASR when
      Atom :: atom(),
      ASR  :: af_event().
   
build_event('receiving') ->
    {event, 'receiving'};
build_event('alarm') ->
    {event, 'alarm'};
build_event('spontaneous') ->
    {event, 'spontaneous'}.


%%--------------------------------------------------------------------
%% @doc
%% Builds the abstract syntax representation for a santoro machine rule.
%% @param
%% @returns
%% @end
%%--------------------------------------------------------------------
-spec build_rule(State, Event, Literal, Actions) -> ASR when
      State   :: af_state(), 
      Event   :: af_event(),
      Literal :: abstract_expr(), 
      Actions :: abstract_expr(),
      ASR     :: af_rule().

build_rule(State, Event, Literal, Actions) ->
    {rule,
     build_state(State), 
     Event, 
     {pattern, Literal},
